...
def calculate_metrics(model, data):
    ...
    for batch in data.test_dataloader():
        with torch.no_grad():
            ...
            if labels is not None:  # change lables to labels_func for function level
                preds_ = torch.softmax(logits[0], dim=1).cpu().numpy() #change logits[0] to logits[1] for function level prediction
                labels_f = labels.cpu().numpy() # change lables to labels_func for function level
...
if not os.path.exists(path=checkpoint_path):
    print(f"[Infos ] --->> Training the model")
    run_id = imp.get_run_id()
    savepath = imp.get_dir(imp.processed_dir() / "codebert" / run_id)
    model = LitGNN( 
                   hfeat= 256,# List of values used  [256, #1024, #512, # 1024 two with 512 and 2 with 1024]
                   embtype= "codebert",
                   methodlevel=False,
                   nsampling=True,
    ...
    dfm.to_csv(f"{imp.outputs_dir()}/metric-evaluation_metrics.csv", index=False)
    prediction_ = calculate_metrics(model, data)[1]
    prediction_.to_csv(f"{imp.outputs_dir()}/model-predict_label.csv", index = False)
