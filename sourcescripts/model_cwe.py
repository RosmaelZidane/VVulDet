...
import utils.cwefeaturemanip as gpht
...
if not os.path.exists(path=checkpoint_path):
    ...
    trainer.save_checkpoint(f"{checkpoint_path}/model-cwe-checkpoint.ckpt")
    # test 
    ...
    # load model
    model = LitGNN.load_from_checkpoint(f"{checkpoint_path}/model-cwe-checkpoint.ckpt")
    # load data
    data = gpht.CVEFixesDatasetLineVDDataModule(
        batch_size=64,
        sample=samplesz,
