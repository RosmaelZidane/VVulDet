...
def CVEFixes(minimal=True, sample=False, return_raw=False, splits="default"):
    """Read CVEFixes Data.
    ...
    if minimal:
        try:
            df = pd.read_parquet(
                savedir / f"minimal_CVEFixes_{sample}.pq", engine="fastparquet"
            ).dropna()

            md = pd.read_csv(imp.cache_dir() / "CVEFixes/CVEFixes_metadata.csv", low_memory=False)
            md.groupby("project").count().sort_values("id")

            default_splits = imp.external_dir() / "CVEFixes_rand_splits.csv"
            ...
            print(E)
            pass
    filename = "sample_Graph.csv" if sample else "domain_CVEFixes-Python.csv"
    ...
    df_savedir = savedir / f"minimal_CVEFixes_{sample}.pq"
    ...
    df[metadata_cols].to_csv(imp.cache_dir() / "CVEFixes/CVEFixes_metadata.csv", index=0)
...
def assign_line_num_to_local(nodes, edges, code):
    ...
    for k, v in twohop_labels.items():
        ...
        types = [i for i in types if i != 442]
        if len(types) == 0:
            continue
...
def feature_extraction(_id, graph_type="cfgcdg", return_nodes=False):
    ...
    _id = svddc.CVEFixesDataset.itempath(177775)
    _id = svddc.CVEFixesDataset.itempath(180189)
    _id = svddc.CVEFixesDataset.itempath(178958)
...
def get_dep_add_lines(filepath_before, filepath_after, added_lines):
    ...
    Example:
    df = svdd.CVEFixes()
    filepath_before = "storage/processed/CVEFixes/before/177775.c"
    filepath_after = "/storage/processed/CVEFixes/after/177775.c"
...
def helper(row):
    ...
    """
    before_path = str(imp.processed_dir() / f"CVEFixes/before/{row['id']}.py")
    after_path = str(imp.processed_dir() / f"CVEFixes/after/{row['id']}.py")
    try:
...
def get_dep_add_lines_CVEFixes(cache=True):
    """Cache dependent added lines for CVEFixes."""
    saved = imp.get_dir(imp.processed_dir() / "CVEFixes/eval") / "statement_labels.pkl"
