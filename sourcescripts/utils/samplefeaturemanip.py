...
class CVEFixesDataset:
    ...
    def __init__(self, partition="train", vulonly=False, sample=-1, splits="default"):
        ...
        self.finished = [
            ...
            for i in glob(str(imp.processed_dir() / "CVEFixes/before/*nodes*"))
        ]
        self.df = prep.CVEFixes(splits=splits)
        ...
        self.df["valid"] = imp.dfmp(
            self.df, CVEFixesDataset.check_validity, "id", desc="Validate Samples: "
        )
    ...
    def check_validity(_id):
        ...
        Example:
        _id = 1320
        with open(str(svd.processed_dir() / f"CVEFixes/before/{_id}.c") + ".nodes.json", "r") as f:
            nodes = json.load(f)
        """
        valid = 0
        try:
            with open(str(CVEFixesDataset.itempath(_id)) + ".nodes.json", "r") as f:
                nodes = json.load(f)
                lineNums = set()
                for n in nodes:
                ...
                if valid == 0:
                    return False
            with open(str(CVEFixesDataset.itempath(_id)) + ".edges.json", "r") as f:
                edges = json.load(f)
            ...
            print(E, str(CVEFixesDataset.itempath(_id)))
            return False
    ...
    def __repr__(self):
        ...
        return f"CVEFixesDataset(partition={self.partition}, samples={len(self)}, vulnperc={vulnperc})"
...
def read_CWESample(_idd):
    ...
    pathd = f"{imp.processed_dir()}/CVEFixes/CWE_Samples"
...
class CVEFixesDatasetLineVD(CVEFixesDataset):
    ...
    def __init__(self, gtype="pdg", feat="all", **kwargs):
        """Init."""
        super(CVEFixesDatasetLineVD, self).__init__(**kwargs)
        lines = prep.get_dep_add_lines_CVEFixes()
    ...
    def item(self, _id, codebert=None):
        """Cache item."""
        savedir = imp.get_dir(
            imp.cache_dir() / f"CVEFixes_linevd_codebert_{self.graph_type}"
        ) / str(_id)
        ...
        code, lineno, ei, eo, et = prep.feature_extraction(
            CVEFixesDataset.itempath(_id), self.graph_type
        )
        ...
        # read the description and optain embedding with codebert: reference functions
...
class CVEFixesDatasetLineVDDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        """Init class from CVEFixes dataset."""
        super().__init__()
