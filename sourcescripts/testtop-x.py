...
def calculate_metrics(model, top1data):
    """
    ...
    ii = top1data['id']
    graphs = []
    graphdir = f"{imp.cache_dir()}/CVEFixes_linevd_codebert_pdg+raw"
    for i in ii:
        try:
            g = load_graphs(f"{graphdir}/{i}")[0][0]
            # print(f"exampel of good graph {i}")
    ...
    return {
        ...
        "pr_auc": pr_auc,
        "mcc": mcc_,
    }, prediction, len(graphs)
    

# find top x CWE-ID in from metadata
# /home/wp3/VVulDet/sourcescripts/storage/cache/CVEFixes/CVEFixes_metadata.csv
metadatapath = imp.cache_dir()
...
# Get top 1
top_1_id = sorted_df.iloc[0]['CWE-ID']
data_top1 = metadata[metadata['CWE-ID'] == top_1_id]
idcwe = list(set(data_top1['CWE-ID']))[0]
